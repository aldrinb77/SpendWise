    ...
    fetch("/api/transactions")
      ...
      .then(json => {
        ...
        if (payload.fallbackToLocal) {
          // No backend connected: Load entirely from LocalStorage
      ...
      .catch(() => {
        // Ultimate fallback if API is fully down
        const localData = localStorage.getItem("spendwise_transactions");
      ...
      {transactions.length > 0 && (
        <div className="flex items-center justify-center py-4">
          <p className="text-sm font-bold text-muted-foreground">
            Showing {transactions.length} transactions {transactions.length > 100 && "(Local Mode)"}
          </p>
        </div>
      )}
    </div>
