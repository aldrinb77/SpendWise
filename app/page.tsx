    ...
    <div className="flex flex-col min-h-screen bg-[#050508] font-ui selection:bg-emerald-500/30 selection:text-white">
      ...
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 flex items-center justify-between gap-12 px-8 py-3.5 rounded-full bg-white/[0.04] border border-white/[0.08] backdrop-blur-3xl shadow-[0_0_80px_rgba(0,0,0,0.5)] min-w-[320px] md:min-w-[600px]">
        ...
        <div className="hidden md:flex items-center gap-10">
          {['Features', 'Dashboard', 'Pricing'].map(item => (
            <a key={item} href={`#${item.toLowerCase()}`} className="text-[10px] font-black uppercase tracking-widest text-white/30 hover:text-white transition-all">{item}</a>
          ))}
        </div>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-[#050508] pt-20">
        ...
        <div className="relative z-10 container mx-auto px-6 pt-24 pb-20 text-center space-y-12">
          ...
          >
            <span className="font-display italic text-white text-7xl md:text-[9.5rem] block">
              Reimagine
            </span>
          ...
          <motion.p
            ...
            className="max-w-2xl mx-auto text-white/40 text-lg md:text-xl leading-relaxed font-medium"
          >
            The only expense tracker that thinks as fast as you spend.<br/>
            AI-categorized transactions, surgical insights, zero noise.
          </motion.p>
          ...
          >
            ...
            <div className="relative rounded-[40px] overflow-hidden border border-white/[0.08] shadow-[0_80px_250px_rgba(0,0,0,0.9)] bg-[#080c14] noise">
              
              {/* Browser Header */}
              <div className="flex items-center justify-between px-8 py-5 border-b border-white/[0.06] bg-white/[0.02] relative">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-3 px-6 py-2 rounded-xl bg-black/40 border border-white/[0.06] w-full max-w-sm">
                  ...
                  <span className="text-white/20 text-[10px] font-mono tracking-tight truncate">spendwise-terminal.network/dashboard</span>
                </div>
              ...
              <div className="p-8 grid grid-cols-12 gap-6 bg-gradient-to-b from-white/[0.01] to-transparent">
                <div className="col-span-2 space-y-2 opacity-40">
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div key={i} className={cn("h-10 rounded-xl", i === 1 ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-white/5")} />
                  ))}
                </div>
                ...
                <div className="col-span-10 space-y-8 text-left">
                  <div className="grid grid-cols-4 gap-4">
                    {[
                      ...
                      <motion.div key={i} className="p-5 rounded-3xl border border-white/[0.06] bg-white/[0.02]">
                        <p className="text-[9px] font-black text-white/20 uppercase tracking-[0.2em] mb-3">{stat.label}</p>
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    <div className="col-span-2 p-6 rounded-3xl border border-white/[0.08] bg-white/[0.02] relative overflow-hidden h-[200px]">
                      <div className="flex items-center justify-between mb-6">
                        <p className="text-white/50 text-[11px] font-black uppercase tracking-widest">Spending Trend</p>
                    ...
                    <div className="col-span-1 p-6 rounded-3xl border border-white/[0.08] bg-white/[0.02]">
                      <p className="text-white/50 text-[11px] font-black uppercase tracking-widest mb-6">Recent</p>
                      {[
                        ...
                        <div key={i} className="flex items-center justify-between mb-4 pb-4 border-b border-white/[0.04] last:border-0 last:mb-0 last:pb-0">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-white/[0.03] border border-white/[0.06]">
                              <div className="w-1.5 h-1.5 rounded-full" style={{ background: tx.color }} />
                            </div>
                            <div>
                              <p className="text-white font-extrabold text-[11px] leading-none mb-1">{tx.name}</p>
                              <p className="text-white/20 text-[9px] font-black uppercase tracking-widest">{tx.cat}</p>
                            </div>
                          </div>
                          <span className={cn("text-[11px] font-black tabular-nums tracking-tighter", tx.amount.startsWith('+') ? 'text-emerald-400' : 'text-white/50')}>
                            {tx.amount}
                          </span>
                        </div>
                      ))}
            ...
            <motion.div
              ...
              className="absolute -top-4 -right-4 bg-emerald-500 text-black text-[10px] font-black px-6 py-2.5 rounded-full shadow-2xl shadow-emerald-500/40 uppercase tracking-[0.2em] relative z-20"
            >
              ✦ Surveillance Active
            </motion.div>
          </motion.div>
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row items-start justify-between gap-20">
            <div className="max-w-xs text-left space-y-6">
              ...
              <div className="flex items-center gap-6">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20">Operational Terminal Node 01</span>
              </div>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-16 md:gap-32 text-left">
              <div className="space-y-6">
                <p className="text-white/30 text-[10px] tracking-[0.3em] uppercase font-black">Structure</p>
                {['Dashboard', 'Ledger', 'AI Insights', 'Inversion'].map(l => (
                  <a key={l} href="#" className="block text-white/40 hover:text-white text-xs font-bold transition-colors">{l}</a>
                ))}
              </div>
              <div className="space-y-6">
                <p className="text-white/30 text-[10px] tracking-[0.3em] uppercase font-black">Protocols</p>
                {['Privacy Policy', 'Terms of Service', 'Security Matrix', 'Contact'].map(l => (
                  <a key={l} href="#" className="block text-white/40 hover:text-white text-xs font-bold transition-colors">{l}</a>
                ))}
              </div>
              <div className="space-y-6 hidden lg:block">
                <p className="text-white/30 text-[10px] tracking-[0.3em] uppercase font-black">Region</p>
                <span className="block text-white/40 text-xs font-bold">Bangalore, IN</span>
                <span className="block text-white/40 text-xs font-bold">Mumbai, IN</span>
                <span className="block text-white/40 text-xs font-bold">Delhi, IN</span>
              </div>
            </div>
          </div>
          <div className="mt-32 pt-12 border-t border-white/[0.04] flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-1.5 grayscale opacity-30">
               <span className="text-[10px] font-black text-white/50 uppercase tracking-widest">Powered by</span>
